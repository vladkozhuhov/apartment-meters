    ...
    <form onSubmit={handleAddReadings} className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center mt-6">
      <div className="bg-white p-6 rounded-lg shadow-lg w-[480px]">
        ...
        <div className="flex items-center justify-between mt-6">
          <button 
            type="submit" 
            className={`${loading ? 'bg-gray-400' : 'bg-green-500 hover:bg-green-600'} text-white px-5 py-2 rounded text-sm font-medium flex items-center`}
            disabled={loading}
          >
            {loading && 
          ...
          <button 
            ...
            onClick={onCancel}
            disabled={loading}
            className={`ml-4 ${loading ? 'bg-gray-300' : 'bg-gray-500 hover:bg-gray-600'} text-white px-5 py-2 rounded text-sm font-medium`}
          >
            Отмена
          </button>
        </div>
