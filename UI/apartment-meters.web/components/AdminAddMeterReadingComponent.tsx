...
interface AdminAddMeterReadingProps {
  onSuccess: () => void;
  onCancel: () => void;
}
...
const AdminAddMeterReadingComponent: React.FC<AdminAddMeterReadingProps> = ({ onSuccess, onCancel }) => {
  ...
  // Добавляем новые состояния для выбора пользователя и даты
  const [apartmentNumber, setApartmentNumber] = useState<string>('');
  const [selectedUserId, setSelectedUserId] = useState<string | null>(null);
  const [selectedUser, setSelectedUser] = useState<UserInfo | null>(null);
  const [readingDate, setReadingDate] = useState<string>(new Date().toISOString().split('T')[0]);
  
  // Метод для поиска пользователя по номеру квартиры
  const searchUserByApartment = async () => {
    if (!apartmentNumber) {
      showError('Введите номер квартиры', 'warning');
      return;
    }
    
    setUserSearching(true);
    try {
      const user = await getUserByApartmentNumber(apartmentNumber);
      if (user) {
        setSelectedUser({
          id: user.id,
        ...
        await fetchWaterMeters(user.id);
      } else {
        showError(`Пользователь с квартирой ${apartmentNumber} не найден`, 'warning');
      ...
      showError('Ошибка при поиске пользователя', 'error');
      setSelectedUser(null);
      setSelectedUserId(null);
      setWaterMeters([]);
    } finally {
  ...
  const fetchWaterMeters = async (userId: string) => {
    try {
      const meters = await getWaterMetersByUserId(userId);
      setWaterMeters(meters);
      
      // Инициализация полей ввода для каждого счетчика
  ...
  // Проверка валидности даты и формата YYYY-MM-DD
  ...
  const handleWholeInputKeyDown = (meterId: string, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key !== 'Tab' && newReadings[meterId]?.whole?.length === 5) {
      ...
      const fractionInput = document.getElementById(`admin-fraction-${meterId}`);
      if (fractionInput) {
        e.preventDefault();
        fractionInput.focus();
      }
  ...
  const handleAddReadings = async (e: React.FormEvent) => {
    ...
    if (!selectedUserId) {
      showError('Не выбран пользователь', 'warning');
      return;
    }
    
    // Проверяем валидность даты
    if (!isDateValid(readingDate)) {
      showError('Указана недопустимая дата', 'warning');
      return;
    }
    ...
    try {
      ...
      const readingsToSubmit = waterMeters.map(meter => {
        ...
        return {
          ...
          differenceValue: 0, // Это поле будет рассчитано на сервере
          readingDate: new Date(readingDate),
        };
      });
      
      // Проверяем и отправляем показания последовательно
      for (const reading of readingsToSubmit) {
        console.log(`Отправка показаний для счетчика ${reading.waterMeterId} со значением ${reading.waterValue} на дату ${reading.readingDate}`);
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-80 backdrop-blur-sm flex justify-center items-center p-4 z-50">
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg w-full max-w-[560px] max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200">
          <h2 className="text-lg sm:text-xl font-bold">Добавление показаний счетчиков</h2>
          <button
            onClick={onCancel}
        ...
        <div className="bg-blue-50 p-3 rounded-lg mb-4">
          <h3 className="text-base font-medium mb-2">Выбор пользователя</h3>
          <div className="flex space-x-2 mb-3">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-l text-sm focus:border-blue-500 focus:outline-none"
              disabled={userSearching || loading}
            />
            <button
              type="button"
              onClick={searchUserByApartment}
              disabled={userSearching || loading}
              className="px-4 py-2 bg-blue-500 text-white rounded-r hover:bg-blue-600 transition-colors duration-200 text-sm"
            >
              {userSearching ? 'Поиск...' : 'Найти'}
            </button>
          </div>
          
          {selectedUser && (
            <div className="bg-white p-3 rounded-lg shadow-sm">
              <p className="font-medium">Выбран пользователь:</p>
              <p>Квартира: <span className="font-semibold">{selectedUser.apartmentNumber}</span></p>
              <p>ФИО: <span className="font-semibold">{selectedUser.lastName} {selectedUser.firstName}</span></p>
            </div>
          )}
        </div>
        ...
        {selectedUserId && (
          <form onSubmit={handleAddReadings}>
            {waterMeters.length === 0 ? (
              <div className="text-center py-4">
                {loading ? 'Загрузка счетчиков...' : 'Нет доступных счетчиков'}
              </div>
            ) : (
              <>
                {Object.entries(groupedMeters).map(([place, meters]) => (
                  <div key={place} className="mb-4 sm:mb-6">
                    <h3 className="text-base sm:text-lg font-medium mb-3 sm:mb-4 pb-2 border-b border-gray-200">{place}</h3>
                    {meters.map((meter) => (
                      <div key={meter.id} className="mb-3 sm:mb-4 ml-1 sm:ml-2">
                        <div className="flex items-center mb-1 sm:mb-2">
                          ...
                          <h4 className="text-sm sm:text-base font-medium text-gray-700">
                            {meter.waterType === 1 ? 'ГВС' : 'Холодное водоснабжение'}
                          </h4>
                        </div>
                        <div className="ml-5 sm:ml-7 flex items-center">
                          <input
                            id={`admin-whole-${meter.id}`}
                            type="text"
                            inputMode="numeric"
            ...
            <div className="flex items-center justify-between mt-4 sm:mt-6">
              ...
              <button 
                ...
                disabled={loading || !selectedUserId || waterMeters.length === 0}
              >
                {loading ? (
                  <>
...
export default AdminAddMeterReadingComponent; 